<?php 	session_start(); 	if (!isset($_SESSION['username'])) {		$_SESSION['msg'] = "You must log in first";		header('location: login.php');	}	if (isset($_GET['logout'])) {		session_destroy();		unset($_SESSION['username']);		header("location: login.php");	}?><?php require('chartdb.php');$query = "SELECT AVG(temperature) AS temperature , dateFROM sensortableGROUP BY date";$result = mysqli_query($conn, $query);$temperaturedata = '';while($row = mysqli_fetch_array($result)){ $temperaturedata .= "{ date:'".$row["date"]."', temperature:".$row["temperature"]."}, ";}$temperaturedata = substr($temperaturedata, 0, -2);?><?phprequire('chartdb.php');$query = "SELECT AVG(humidity) AS humidity , dateFROM sensortableGROUP BY date";$result = mysqli_query($conn, $query);$humiditydata = '';while($row = mysqli_fetch_array($result)){ $humiditydata .= "{ date:'".$row["date"]."', humidity:".$row["humidity"]."}, ";}$humiditydata = substr($humiditydata, 0, -2);?>
<!DOCTYPE html>
